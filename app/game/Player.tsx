    ...
    <Hud>
      ...
      >
        <mesh ref={globeRef}>
          ...
          <instancedMesh
            ref={stoneMeshRef}
            args={[undefined as any, undefined as any, BATU_DATA.length]}
          >
          ...
          <instancedMesh
            ...
            args={[
              ...
              undefined as any,
              KOMODO_DATA.length +
                ORANGUTAN_DATA.length +
                RAJAWALI_DATA.length +
                BADAK_DATA.length,
            ]}
          >
            <boxGeometry args={[0.08, 0.08, 0.08]} />
...
export default function Player({
  ...
  const audioRefs = useRef<{
    walk1: HTMLAudioElement;
    walk2: HTMLAudioElement;
  } | null>(null);
  useEffect(() => {
    audioRefs.current = {
  ...
  useFrame((_, timeDelta) => {
    ...
    if (
      ...
      runningActionRef.current =
        ...
        actions["Run"] ||
        actions["run"] ||
        actions[keys.find((k) => k.toLowerCase().includes("run")) || ""] ||
        null;
      idleActionRef.current =
        actions["idle"] ||
        actions["Idle"] ||
        actions["idle_01"] ||
        actions[keys.find((k) => k.toLowerCase().includes("idle")) || ""] ||
        null;
    ...
    if (actionsResolvedRef.current) {
      ...
      if (shouldRun !== prevIsMovingAnim.current) {
        ...
        const targetAction = shouldRun
          ? runningActionRef.current
          : idleActionRef.current;
        const prevAction = shouldRun
          ? idleActionRef.current
          : runningActionRef.current;
    ...
    if (isMoving && currentMenuState === "playing") {
      ...
      if (walkTimer.current > 0.35) {
        walkTimer.current = 0;
        if (audioRefs.current) {
          const stepAudio = stepToggle.current
            ? audioRefs.current.walk1
            : audioRefs.current.walk2;
    ...
    if (isMoving && currentMenuState === "playing") {
      _nextPos
        .copy(playerPosition.current)
        .addScaledVector(_inputDir, speed * delta);
      ...
      for (let i = 0; i < COLLIDERS.length; i++) {
        ...
        const distSq =
          _p1.distanceToSquared(_p2) * PLANET_RADIUS * PLANET_RADIUS;
    ...
    if (proximityTimer.current >= 0.1) {
      ...
      for (let i = 0; i < COLLIDERS.length; i++) {
        const col = COLLIDERS[i];
        if (col.type !== "animal" && col.type !== "stone" && col.type !== "npc")
          continue;
      ...
      if (closestAnimalId !== lastNearbyId.current) {
        ...
        if (closestAnimalId !== null) {
          const matchingCreature =
            NUSA_CREATURES.find((c) => c.id === closestAnimalId) || null;
    ...
    if (lightGroupRef.current) {
      ...
      lightGroupRef.current.quaternion.setFromUnitVectors(
  ...
  return (
    <>
      ...
      <CartoonSmoke playerPosition={playerPosition} isMovingRef={isMovingRef} />

      {menuState === "playing" && (
        <MinimapGlobe playerPosition={playerPosition} />
      )}
    </>
  );
}
